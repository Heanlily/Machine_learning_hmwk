...
DATA_PATH = "D:\PPT\\second\ML\\hmwk2\\adult" #TODO: if you are working somewhere other than the csug server, change this to the directory where a7a.train, a7a.dev, and a7a.test are on your machine

#returns the label and feature value vector for one datapont (represented as a line (string) from the data file)
def parse_line(line):
    tokens = line.split()
    ...
    for n in range(args.iterations):
        ...
        for i in range(len(train_ys)):
            if (np.dot(train_xs[i],weights)*train_ys[i])<=0:
                weights+=args.lr*np.dot(train_xs[i],train_ys[i])
                false_num+=1
        weights_list.append(weights)
        false_list.append(false_num)
    false_list = np.array(false_list)
    indexx = int(np.argmin(false_list))
    weights= weights_list[indexx]

    test_accuracy(weights,dev_ys,dev_xs)
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    for i in range(np.size(test_ys)):
